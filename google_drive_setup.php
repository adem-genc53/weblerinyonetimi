...
function getClient() {
    try {
        ...
        file_put_contents(KOKYOLU . 'error.log', date('Y-m-d H:i:s') . ' - ' . basename(__FILE__) . ' - Bağlantı başarısız, bilgileri kontrol edin 2: ' .  $e->getMessage() . ' | ' . $e->getLine() . "\n", FILE_APPEND);
        return null;
    } catch (Exception $e) {
        echo "Beklenmeyen bir hata oluştu: " . $e->getMessage();
        file_put_contents(KOKYOLU . 'error.log', date('Y-m-d H:i:s') . ' - ' . basename(__FILE__) . ' - Beklenmeyen bir hata oluştu: ' .  $e->getMessage() . ' | ' . $e->getLine() .  "\n", FILE_APPEND);
        return null;
    }
}
...
try {
    ...
    if ($client) {
        try {
            ...
            file_put_contents(KOKYOLU . 'error.log', date('Y-m-d H:i:s') . ' - ' . basename(__FILE__) . ' - Google Drive hizmetine bağlanırken bir hata oluştu: ' .  $e->getMessage() . ' | ' . $e->getLine() . "\n", FILE_APPEND);
        } catch (Exception $e) {
            echo "Beklenmeyen bir hata oluştu: " . $e->getMessage();
            file_put_contents(KOKYOLU . 'error.log', date('Y-m-d H:i:s') . ' - ' . basename(__FILE__) . ' - Beklenmeyen bir hata oluştu: ' .  $e->getMessage() . ' | ' . $e->getLine() . "\n", FILE_APPEND);
        }
        ...
        file_put_contents(KOKYOLU . 'error.log', date('Y-m-d H:i:s') . ' - ' . basename(__FILE__) . ' - Google Client oluşturulamadı | ' . $e->getLine() .  "\n", FILE_APPEND);
    }
} catch (Exception $e) {
    echo "Beklenmeyen bir hata oluştu (dış try-catch): " . $e->getMessage();
    file_put_contents(KOKYOLU . 'error.log', date('Y-m-d H:i:s') . ' - ' . basename(__FILE__) . ' - Beklenmeyen bir hata oluştu (dış try-catch): ' .  $e->getMessage() . ' | ' . $e->getLine() . "\n", FILE_APPEND);
}
